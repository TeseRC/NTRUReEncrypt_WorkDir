...
using namespace ns3;
using namespace std;
NS_LOG_COMPONENT_DEFINE ("VANET_AODV");
...
void PktTrans2(NodeContainer b,NodeContainer c, NodeContainer d){
	std::cout<<"\n  Select the path between the nodes by using  AD-HOC ON-DEMAND DISTANCE VECTOR ROUTING PROTOCOL(AODV Routing Protocol).  \n\n";
	for(uint32_t i=0; i<b.GetN (); i++){
		TypeId tid1 = TypeId::LookupByName ("ns3::UdpSocketFactory");
		Ptr<Socket> recvSink1 = Socket::CreateSocket (d.Get (0), tid1);
		InetSocketAddress local1 = InetSocketAddress (Ipv4Address::GetAny (), 80);
...
int main (int argc, char *argv[]){
	...
	cmd.Parse (argc, argv); 
	// create 3 node containers, for group A, group B and RSUs
	NodeContainer Group_A_Vehicles;
	NodeContainer Group_B_Vehicles;
	NodeContainer RSUNodes;
	std::cout<<"\n  The Network consists of " << numRSU <<"- RSUs and "<< numGroup_A_Nodes+numGroup_B_Nodes  <<"- Vehicles based on the random wave point mobility model.  \n";
	Group_A_Vehicles.Create (numGroup_A_Nodes);
	Group_B_Vehicles.Create (numGroup_B_Nodes);
	RSUNodes.Create (numRSU);
	
	// set WAVE parameters, based on the wifi module
	WifiHelper wifi;
	...
	// install the WAVE net devices in all nodes
	...
	// use WAVE helper to set the wifi channel as WAVE using the 802.11p standard
	...
	// define the simulation area and set node positions and mobility
	...
	// RWP mobility for vehicles
	mobility.Install (Group_B_Vehicles);
	mobility.Install (Group_A_Vehicles);
	MobilityHelper mobility1;
	Simulator::Schedule (Seconds (0.3), &getNearbynodesrc,Group_B_Vehicles); // just for animation purposes
	mobility1.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
	// Constant position for RSUs
	mobility1.Install (RSUNodes);
	// Animation class to build the XML files to use with netAnim
	...
	// use AODV routing protocol to disseminate packets
	AodvHelper aodv;
	...
	// assign IPv4 addresses to all node interfaces
	...
	macgplot mg;
	mg.Delay((int)numGroup_A_Nodes+numGroup_B_Nodes,"AODV");
	mg.Packet_Loss_Ratio((int)numGroup_A_Nodes+numGroup_B_Nodes,"AODV");
	mg.Throughput((int)numGroup_A_Nodes+numGroup_B_Nodes,"AODV");
	mg.Packet_Delivery_Ratio((int)numGroup_A_Nodes+numGroup_B_Nodes,"AODV");
	pAnim= new AnimationInterface ("VANET_AODV.xml");
	...
	for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i){
	    ...
	    NS_LOG_UNCOND("Throughput: " << i->second.rxBytes * 8.0 / (i->second.timeLastRxPacket.GetSeconds()-i->second.timeFirstTxPacket.GetSeconds()) / 512  << " Kbps");
	    txTot += i->second.txPackets;
	    rxTot += i->second.rxPackets;
	}

    NS_LOG_UNCOND("TxTot Packets= " << txTot);
    NS_LOG_UNCOND("RxTot Packets= " << rxTot);
  	monitor->SerializeToXmlFile("VANET_AODV.flowmon", true, true);
